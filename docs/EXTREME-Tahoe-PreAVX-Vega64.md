...
> **MC:** J detect INTEGRATE 본 패스 · **Sweep:** `tahoe_gate` / L5·M gate wire (race 분리)
...
| **J detect** | **integrated** (stage `3800bf8`/`393192d` → live) | **MC 본 패스** |
| **`tahoe_gate.py`** | 워킹트리 untracked · L5/M dirty가 import 중 | **Sweep** |
| **L5 → `is_tahoe`** | live WIP | **Sweep** |
| **M → `tahoe_gate`** | WIP | **Sweep** |
...
2. ✅ **J** detect soft-merge (본 패스)
3. **Sweep:** `tahoe_gate` land + L5/M gate wire  
