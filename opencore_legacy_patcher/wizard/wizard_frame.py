...
class WizardFrame(wx.Frame):
    """
    일반 사용자용 단계별 마법사 UI.
    고급 기능은 gui_main_menu.MainFrame(고급 모드)로 전환합니다.
    """
    ...
    def __init__(
        ...
        logging.info("26x86 마법사 프레임 초기화")
        super().__init__(
            parent,
            title=title,
            size=(820, 620),
    ...
    def _build_layout(self) -> None:
        ...
        subtitle = wx.StaticText(header, label="오래된 Mac, 새 macOS")
        subtitle.SetFont(gui_support.font_factory(12, wx.FONTWEIGHT_NORMAL))
    ...
    def _on_root_unpatch(self, event: wx.Event = None) -> None:
        self.Hide()
        try:
            from ..wx_gui import gui_sys_patch_start
    ...
    def _on_about(self, event: wx.Event = None) -> None:
        ...
        gui_about.AboutFrame(self.constants)
