...
| 파일 / 디렉터리 | 사유 |
|-----------------|------|
| `EFI_COMPARISON_16048_vs_16903.txt` | OpenCore 빌드 4.0.0.16048 vs 4.0.0.16903 EFI 설정 비교 일회성 로그 (MacBookPro14,3 / Tahoe) |
| `NEW_EFI_REPORT.txt` | EFI 빌드 테스트 리포트 |
| `PREVIOUS_BUILD_STATE.txt` | 이전 빌드 상태 스냅샷 |
| `NEW_EFI_4.0.0.16903/` | 테스트용 EFI 트리 (`EFI/OC/config.plist` 등, 약 10MB) |
| `verify_efi.py` | 루트 EFI 검증 Python 스크립트; 운영 도구는 `Tools/verify_efi.command` (bash) 사용 |
...
| `replace_gui_sys_patch_display.py` | sys_patch GUI 표시 일회성 치환 스크립트 |
| `Install-OCLP-T1-MBP143-to-USB.command` | 레거시 OCLP 이름·이탈리아어 USB 설치 스크립트 |
| `Install-OCLP-T1-MBP143-to-USB.sh` | 동일 (셸 버전) |
...
필요 시 `archive/`에서 프로젝트 루트로 파일을 되돌릴 수 있습니다. 권장하지는 않으며, 참고·비교 목적으로만 보관합니다.
