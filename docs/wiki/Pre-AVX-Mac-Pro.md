...
> WindowServer 노란 화면 완전 해결은 Phase 1 범위 밖입니다. 이 문서는 **감지 · EFI 자동 패치 · Metal 분기 훅**까지만 다룹니다.
...
| `has_avx2` | AVX2 (`machdep.cpu.leaf7_features`) 존재 여부 |
| `safari_pre_avx_fix_recommended` | Safari26 RestrictEvents 교체 권장 여부 |
| `auto_pre_avx_patch` | 사용자 설정 (기본 `true`) |
| `recommended_tahoe_graphics_policy` | Tahoe 루트 GPU 패치 정책 |
...
- **WindowServer 노란 화면** — 미해결 (Layer B R&D)
